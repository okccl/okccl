...
flowchart LR
  ...
  subgraph GH["GitHub"]
    ...
    PlatformRepo["platform-gitops\n(ミドルウェア)"]
    AppsRepo["apps-gitops\n(アプリ)"]
  end
  ...
  CI -->|image push| GHCR
  CI --> CD
  CD -->|image tag PR| AppsRepo
  Backstage -.->|環境払い出し PR| AppsRepo
  PlatformRepo -->|sync| ArgoCD
  AppsRepo -->|sync| ArgoCD
  ArgoCD -->|apply| Platform
  ArgoCD -->|apply| Apps
